...
#ifdef RESOURCES_VIRTIO
static uint32 Stm32VirtioSerialInit(struct VirtioDriver *virtio_drv, struct BusConfigureInfo *configure_info)
{
    NULL_PARAM_CHECK(virtio_drv);

    struct SerialCfgParam *serial_cfg = (struct SerialCfgParam *)virtio_drv->private_data;
    struct UsartHwCfg *serial_hw_cfg = (struct UsartHwCfg *)serial_cfg->hw_cfg.private_data;
...
#ifdef RESOURCES_VIRTIO
static uint32 Stm32VirtioSerialConfigure(struct VirtioDriver *virtio_drv, int serial_operation_cmd)
{
    NULL_PARAM_CHECK(virtio_drv);

    struct VirtioHardwareDevice *virtio_dev = (struct VirtioHardwareDevice *)virtio_drv->driver.owner_bus->owner_haldev;
    struct SerialCfgParam *serial_cfg = (struct SerialCfgParam *)virtio_drv->private_data;
    struct UsartHwCfg *serial_hw_cfg = (struct UsartHwCfg *)serial_cfg->hw_cfg.private_data;
    struct SerialDevParam *serial_dev_param = (struct SerialDevParam *)virtio_dev->haldev.private_data;
    ...
    switch (serial_operation_cmd)
    {
        ...
        case OPER_SET_INT:
            ...
            break;
        case OPER_CONFIG :
            KPrintf("DMA not supported yet.\n");
            break;
            // if (SIGN_OPER_DMA_RX == serial_dev_param->serial_set_mode) {
            //     DMAConfiguration(virtio_dev, serial_hw_cfg->uart_device);
            // }
    }
...
static int Stm32VirtioSerialPutchar(struct VirtioHardwareDevice *virtio_dev, char c)
{
    struct SerialCfgParam *serial_cfg = (struct SerialCfgParam *)virtio_dev->private_data;
    struct UsartHwCfg *serial_hw_cfg = (struct UsartHwCfg *)serial_cfg->hw_cfg.private_data;
...
static int Stm32VirtioSerialGetchar(struct VirtioHardwareDevice *virtio_dev)
{
    struct SerialCfgParam *serial_cfg = (struct SerialCfgParam *)virtio_dev->private_data;
    struct UsartHwCfg *serial_hw_cfg = (struct UsartHwCfg *)serial_cfg->hw_cfg.private_data;
...
#ifdef RESOURCES_VIRTIO
static void UartVirtioIsr(struct VirtioDriver *virtio_drv, struct VirtioHardwareDevice *virtio_dev)
{
    // struct Stm32UsartDma *dma = &serial->dma; 
    struct SerialCfgParam *serial_cfg = (struct SerialCfgParam *)virtio_drv->private_data;
    struct UsartHwCfg *serial_hw_cfg = (struct UsartHwCfg *)serial_cfg->hw_cfg.private_data;
    ...
    if (USART_GetITStatus(serial_hw_cfg->uart_device, USART_IT_RXNE) != RESET) {
        VirtioSetIsr((void *)virtio_dev, SERIAL_EVENT_RX_IND);
        USART_ClearITPendingBit(serial_hw_cfg->uart_device, USART_IT_RXNE);
    } else if (USART_GetITStatus(serial_hw_cfg->uart_device, USART_IT_IDLE) != RESET) {
        // DmaUartRxIdleIsr(serial_dev, dma, serial_hw_cfg->uart_device);
        KPrintf("UartVirtioIsr: Should not be here, not supported yet\n");
    } else if (USART_GetITStatus(serial_hw_cfg->uart_device, USART_IT_TC) != RESET) {
        USART_ClearITPendingBit(serial_hw_cfg->uart_device, USART_IT_TC);
    } else {
        if (USART_GetFlagStatus(serial_hw_cfg->uart_device, USART_FLAG_ORE) == SET) {
...
#ifdef RESOURCES_VIRTIO
struct VirtioBus virtio_bus;
struct VirtioDriver virtio_driver;
struct VirtioHardwareDevice virtio_device;
#define VIRTIO_BUS_NAME "virtio_bus"
#define VIRTIO_DRV_NAME "virtio_drv"
#define VIRTIO_DEVICE_NAME "virtio_dev"

// TODO: irq handler
void USART2_VIRTIO_IRQHandler(int irq_num, void *arg)
{
    UartVirtioIsr(&virtio_driver, &virtio_device);
}
DECLARE_HW_IRQ(USART2_IRQn, USART2_VIRTIO_IRQHandler, NONE);
...
#ifdef RESOURCES_VIRTIO
static uint32 Stm32VirtioSerialDrvConfigure(void *drv, struct BusConfigureInfo *configure_info)
{
    NULL_PARAM_CHECK(drv);
    NULL_PARAM_CHECK(configure_info);
    ...
    {
        case OPE_INT:
            ret = Stm32VirtioSerialInit(virtio_drv, configure_info);
            break;
        case OPE_CFG:
            serial_operation_cmd = *(int *)configure_info->private_data;
            ret = Stm32VirtioSerialConfigure(virtio_drv, serial_operation_cmd);
            break;
        default:
            break;
    }
...
{
    .init = Stm32VirtioSerialInit,
    .configure = Stm32VirtioSerialConfigure,
};

/*manage the serial device hal operations*/
static struct VirtioHwDevDone hwdev_done_virtio =
{
    .put_char = Stm32VirtioSerialPutchar,
    .get_char = Stm32VirtioSerialGetchar,
};

static int BoardVirtioSerialBusInit(struct VirtioBus *virtio_bus, struct VirtioDriver *virtio_driver, const char *bus_name, const char *drv_name)
{
    x_err_t ret = EOK;
    ...
    if (EOK != ret) {
        KPrintf("hw_virtio_init VirtioBusInit error %d\n", ret);
        return ERROR;
    }
    ...
    if (EOK != ret) {
        KPrintf("hw_virtio_init VirtioDriverInit error %d\n", ret);
        return ERROR;
    }
    ...
    if (EOK != ret) {
        KPrintf("hw_virtio_init VirtioDriverAttachToBus error %d\n", ret);
        return ERROR;
    } 
...
static int BoardVirtioSerialDevBend(struct VirtioHardwareDevice *virtio_device, void *serial_param, const char *bus_name, const char *dev_name)
{
    x_err_t ret = EOK;

    ret = VirtioDeviceRegister(virtio_device, serial_param, dev_name);
    if (EOK != ret) {
        KPrintf("hw_virtio_init VirtioDeviceInit device %s error %d\n", dev_name, ret);
        return ERROR;
    }  

    ret = VirtioDeviceAttachToBus(dev_name, bus_name);
    if (EOK != ret) {
        KPrintf("hw_virtio_init VirtioDeviceAttachToBus device %s error %d\n", dev_name, ret);
        return ERROR;
    }  
...
int Stm32HwUsartInit(void)
{
...
#ifdef RESOURCES_VIRTIO
    static struct SerialCfgParam serial_cfg_virtio;
    memset(&serial_cfg_virtio, 0, sizeof(struct SerialCfgParam));

    static struct UsartHwCfg serial_hw_cfg_virtio;
    memset(&serial_hw_cfg_virtio, 0, sizeof(struct UsartHwCfg));

    static struct SerialDevParam serial_dev_param_virtio;
    memset(&serial_dev_param_virtio, 0, sizeof(struct SerialDevParam));
    ...
    virtio_driver.configure = &Stm32VirtioSerialDrvConfigure;
    virtio_device.hwdev_done = &hwdev_done_virtio;

    serial_cfg_virtio.data_cfg = data_cfg_init;

    serial_hw_cfg_virtio.uart_device = USART2;
    serial_hw_cfg_virtio.irq = USART2_IRQn;
    serial_cfg_virtio.hw_cfg.private_data = (void *)&serial_hw_cfg_virtio;
    virtio_driver.private_data = (void *)&serial_cfg_virtio;
    ...
    serial_dev_param_virtio.serial_work_mode = SIGN_OPER_INT_RX;
#ifdef USART2_USING_DMA
    serial_dev_param_virtio.serial_work_mode = SIGN_OPER_DMA_RX;
#endif
    virtio_device.haldev.private_data = (void *)&serial_dev_param_virtio;

    NVIC_Configuration(serial_hw_cfg_virtio.irq);

    ret = BoardVirtioSerialBusInit(&virtio_bus, &virtio_driver, VIRTIO_BUS_NAME, VIRTIO_DRV_NAME);
    if (EOK != ret) {
        KPrintf("Stm32HwUsartInit usart2 error ret %u\n", ret);
        return ERROR;
    }

    ret = BoardVirtioSerialDevBend(&virtio_device, (void *)&serial_cfg_virtio, VIRTIO_BUS_NAME, VIRTIO_DEVICE_NAME);
    if (EOK != ret) {
        KPrintf("Stm32HwUsartInit usart2 error ret %u\n", ret);
        return ERROR;
    }    

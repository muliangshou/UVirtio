...
#ifdef DEBUG_UART1
static HardwareDevType _serial_log = NONE;

HardwareDevType ObtainSerialLog(void)
{
    return _serial_log;
}
#endif
...
#ifdef DEBUG_UART1
HardwareDevType InstallSerialLog(const char *bus_name, const char *drv_name, const char *dev_name)
{
    // NOTE: should be called after memory initialize
    BusType serial_log_bus;
    DriverType serial_log_drv = NONE;
    HardwareDevType serial_log = NONE;
    struct SerialDevParam *serial_log_param = NONE;
    struct SerialCfgParam serial_log_cfg;
    struct BusConfigureInfo configure_info;
    ...
    serial_log_bus = BusFind(bus_name);
    serial_log_drv = BusFindDriver(serial_log_bus, drv_name);
    serial_log = BusFindDevice(serial_log_bus, dev_name);

    if (serial_log != NONE) {
        if (_serial_log != NONE) {
            BusDevClose(_serial_log);
        }

        configure_info.configure_cmd = OPE_INT;
        memset(&serial_log_cfg, 0, sizeof(struct SerialCfgParam));
        configure_info.private_data = &serial_log_cfg;
        BusDrvConfigure(serial_log_drv, &configure_info);
        serial_log_bus->match(serial_log_drv, serial_log);

        serial_log_param = (struct SerialDevParam *)serial_log->private_data;
        serial_log_param->serial_set_mode = 0;
        serial_log_param->serial_stream_mode = SIGN_OPER_STREAM;

        BusDevOpen(serial_log);
        _serial_log = serial_log;
    } else {
        serial_log = _serial_log;
    }
...
HardwareDevType InstallVirtioSerial(const char *bus_name, const char *drv_name, const char *dev_name)
{
    BusType virtio_bus;
    DriverType virtio_drv = NONE;
    HardwareDevType virtio_serial = NONE;
    // TODO: not implemented
    // struct SerialDevParam *virtio_dev_param = NONE;
    struct SerialCfgParam virtio_cfg;
    struct BusConfigureInfo configure_info;
    ...
    virtio_bus = BusFind(bus_name);
    virtio_drv = BusFindDriver(virtio_bus, drv_name);
    virtio_serial = BusFindDevice(virtio_bus, dev_name);

    if (virtio_serial != NONE) {
        // if (_console != NONE) {
        //     BusDevClose(_console);
        // }
        ...
        // TODO: not implemented
        configure_info.configure_cmd = OPE_INT;
        memset(&virtio_cfg, 0, sizeof(struct SerialCfgParam));
        configure_info.private_data = &virtio_cfg;
        BusDrvConfigure(virtio_drv, &configure_info);
        virtio_bus->match(virtio_drv, virtio_serial);

        // serial_dev_param = (struct SerialDevParam *)virtio_serial->private_data;
        // serial_dev_param->serial_set_mode = 0;
        // serial_dev_param->serial_stream_mode = SIGN_OPER_STREAM;
        // malloc error
        BusDevOpen(virtio_serial);
        // _console = console;
    } else {
        // console = _console;
    }

    return virtio_serial;
}
#endif

...
{
    // KPrintf("Dummy\n");
    BusType virtio_bus = BusFind("virtio_bus");
    HardwareDevType virtio_serial = BusFindDevice(virtio_bus, "virtio_dev");
    // KPrintf("%d parameter(s)\n", argc);
    // for (int i = 1; i < argc; ++i){
    //     KPrintf("%d parameter: %s \r\n", i, argv[i]);
    // }
    if (argc != 2) {
        KPrintf("Error: wrong number of arguments, expect 1, got %d\n", argc-1);
        return;
    }
    
    char *dummy = argv[1];
    int cnt = 1;
    KPrintf("write data to virtio serial\n");
    struct BusBlockWriteParam write_param;
    write_param.size = strlen(dummy);
    write_param.buffer = dummy;
    ...
    // unsigned long start_mtime = 0, end_mtime = 0;
    ...
    for (int i = 0; i < cnt; i++){
        // BusDevWriteData(virtio_serial, &write_param);
        BusDevWriteData(virtio_serial, &write_param);
    }
    ...
    // KPrintf("write %d bytes for %d times\nstart: %ld, end: %ld, delta_time: %ld\n", strlen(dummy), cnt, start_mtime, end_mtime, end_mtime-start_mtime);
    KPrintf("write %d bytes to virtio serial\n", strlen(dummy));

    // KPrintf("read %d bytes from virtio serial: %s##\n", read_param.read_length, data);

}
SHELL_EXPORT_CMD(SHELL_CMD_PERMISSION(0)|SHELL_CMD_TYPE(SHELL_TYPE_CMD_MAIN)|SHELL_CMD_DISABLE_RETURN, Test1, Test1, test1);
SHELL_EXPORT_CMD(SHELL_CMD_PERMISSION(0)|SHELL_CMD_TYPE(SHELL_TYPE_CMD_MAIN)|SHELL_CMD_DISABLE_RETURN, vs_write, Test1, test1);
...
void Test2(int argc, char *argv[])
{
    ...
    BusDevReadData(virtio_serial, &read_param);

    // KPrintf("read %d bytes from virtio serial: %s##\n", read_param.read_length, data);
    
    // unsigned long end_mtime;
    //     unsigned long tmp = get_timer_value();
    // do {
    //     end_mtime = get_timer_value();
    // } while (end_mtime == tmp);
...
SHELL_EXPORT_CMD(SHELL_CMD_PERMISSION(0)|SHELL_CMD_TYPE(SHELL_TYPE_CMD_MAIN)|SHELL_CMD_DISABLE_RETURN, Test2, Test2, test2);
SHELL_EXPORT_CMD(SHELL_CMD_PERMISSION(0)|SHELL_CMD_TYPE(SHELL_TYPE_CMD_MAIN)|SHELL_CMD_DISABLE_RETURN, vs_read, Test2, test2);
...
{
    int32 tid = KTaskCreate("Test task",
                        test_task, NONE,
...
SHELL_EXPORT_CMD(SHELL_CMD_PERMISSION(0)|SHELL_CMD_TYPE(SHELL_TYPE_CMD_MAIN)|SHELL_CMD_DISABLE_RETURN, Test3, Test3, test3);